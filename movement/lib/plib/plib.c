...
static char nakshatra_short[][7]={"Aswini","Bharni","Kritka","Rohini","Mrgsra","Ardhra",
		      ...
		      "Hasta","Chitra","Swathi","Visaka","Anrada","Jyesht","Mula",
		      "PAsada","UAsada","Sravan","Dnista","Satbsa",
		      "PBadra","UBadra","Revthi"};

static float Ls, Lm, Ms, Mm;

//Calculate Aynamsa
static float calc_ayanamsa(float d)
{
	float t, o, l, ayan;
...
{
	float d, ayanamsa, slon, mlon, tmlon, tslon;
	int n;
	...
	printf("Panchang of %d-%d-%d %fh z%fh\n", dd, mm, yy, hr, zhr);
	printf("slon = %f mlon =%f\n", slon, mlon);
	...
	pdata->tithi = n+1;
	(n <= 14) ? strcpy(pdata->dpaksha,"Su") : strcpy(pdata->dpaksha,"kr");

	//Calculate Nakshatra
	tmlon = REV(mlon+ayanamsa);
	// pdata->nakshatra = (int)(tmlon*6/80);
	strcpy(pdata->dnakshatra, nakshatra_short[(int)(tmlon*6/80)]);
